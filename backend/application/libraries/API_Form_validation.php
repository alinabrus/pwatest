 ...
 */
class API_Form_validation extends CI_Form_validation {
	 ...
	 * Constructor
	 */
	public function __construct(array $post = null, $rules = array())
	{
	...
	public function run_custom (array $post, array $rules = array())
	{
		...
		*/ 
		/*
		logmes('<<<<<----------------------------','','aaa');
		logmes('errors: ',$this->_error_array,'aaa');
		logmes('$post = ',$post,'aaa');
		logmes('$rules = ',$rules,'aaa'); */
		$result =  $this->run();
		/*
		logmes('$result = ',$result,'aaa');
		logmes('errors: ',$this->_error_array,'aaa');
		logmes('$this->post = ',$this->post,'aaa');
		logmes('$this->_config_rules = ',$this->_config_rules,'aaa');
		logmes('---------------------------->>>>>>','','aaa');
		*/
		return $result;
	}
	...
	{
		//log_message('debug', "Form Validation post = ".var_export($this->post, true));
		//log_message('debug', "Form Validation post count = ".count($this->post));
	...
	{
		//$emailRegexp_ci = "/^([a-z0-9\+_\-]+)(\.[a-z0-9\+_\-]+)*@([a-z0-9\-]+\.)+[a-z]{2,6}$/ix";
		//$emailRegexp_ng = "/^[a-z0-9!#$%&'*+/=?^_`{|}~.-]+@[a-z0-9-]+(\.[a-z0-9-]+)*$/i";
		$emailRegexp = "/^([a-z0-9\+_\-]+)(\.[a-z0-9\+_\-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*$/ix";
		return ( ! preg_match($emailRegexp, $str)) ? FALSE : TRUE;
	}
