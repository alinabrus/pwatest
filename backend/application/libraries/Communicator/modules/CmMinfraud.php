...
require_once CM_LIBS_PATH.'external/ccfd-api-php-master/src/CreditCardFraudDetection.php';

class CmMinfraud extends CmModule
{
	...
	public function fraud_detection(array $input = array()) 
	{
		$input['requested_type'] = 'standard'; //'standard' or 'premium'
		
		$this->logger->log(__METHOD__.". Params: ", $input);
		...
		//try {
			...
			$input["license_key"] = $this->config['license_key'];
			
			// Required fields
			//$input['i']       = '93.114.43.244';        // set the client ip address
			/*
			...
			if (empty($response['err']) 
				|| in_array($response['err'], $this->warningCodes) 
				|| $response['err'] == 'IP_NOT_FOUND'
			){ 
				$this->logger->log("Communicator fraud_detection request Warning in module ". get_class($this) .". ". $response['err'].". Params: ", $input);
			}
			else {
				//throw new Exception($response['err']);
				$this->logger->log("Communicator fraud_detection request Error in module ". get_class($this) .". ". $response['err'].". Params: ", $input);
			}
			
			if ( ! empty($response['err']) && in_array($response['err'], $this->errCodes)) 
				$response['error'] = $response['err'];
		/*
		}
		catch (Exception $e) {
			$this->logger->log("Communicator fraud_detection request Error in module ". get_class($this) .". ". $e->getMessage().". Params: ", $input);
			//throw $e;
