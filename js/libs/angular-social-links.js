(function() {
  ...
  angular.module('socialLinks', []).factory('socialLinker', [
    ...
    'socialLinker', function(linker) {
      return {
        restrict: 'ACEM',
        fbScope: sharedScopeDefinition,
        link: linker(function(fbScope, url) {
          var shareUrl;
    ...
    'socialLinker', function(linker) {
      return {
        restrict: 'ACEM',
        twScope: angular.extend({
          status: '@status'
        }, sharedScopeDefinition),
        link: linker(function(twScope, url) {
          twScope.status || (twScope.status = "Check this out! - " + url);
          return "https://twitter.com/intent/tweet?text=" + (encodeURIComponent(twScope.status));
        })
      };
    }
